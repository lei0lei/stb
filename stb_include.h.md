...
解说：这一段声明结构体 `include_info`，把相关字段组织成一个数据对象，供后续算法在函数之间传递和维护状态。
...
解说：这一段实现或声明库接口 `stb_include_free_includes`，把“include free includes”相关能力封装成可被外部或内部复用的函数。
...
解说：这一段实现或声明库接口 `stb_include_isspace`，把“include isspace”相关能力封装成可被外部或内部复用的函数。
...
解说：这一段实现或声明库接口 `stb_include_find_includes`，把“include find includes”相关能力封装成可被外部或内部复用的函数。
...
解说：这一段实现或声明库接口 `stb_include_itoa`，把“include itoa”相关能力封装成可被外部或内部复用的函数。
